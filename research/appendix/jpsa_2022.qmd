...
bind_rows(list("I_P"  = es1_w1,
  ...
  tab_row_group(rows = (Outcome == "投票意向"), 
                label = "投票意向 (SD = 0.565)") |>
  cols_hide(columns = "Outcome") |>
  tab_footnote("注: 標準偏差は性別・年齢・地域で重み付け")
```
...
bind_rows(list("I_P"  = es1_w2,
  ...
  tab_row_group(rows = (Outcome == "投票意向"), 
                label = "投票意向 (SD = 0.644)") |>
  cols_hide(columns = "Outcome") |>
  tab_footnote("注: 標準偏差は2022年6月の統一地方選挙の投票有無で重み付け")
```
...
bind_rows(list("I_P"  = es1_w3,
  ...
  tab_row_group(rows = (Outcome == "投票意向"), 
                label = "投票意向 (SD = 0.711)") |>
  cols_hide(columns = "Outcome") |>
  tab_footnote("注: 標準偏差は2022年3月大統領選挙の投票先で重み付け")
```
