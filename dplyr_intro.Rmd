...
* 修正履歴
    * 2020/06/28: 公開
    * 2020/07/01: データの結合を追加
    * 2020/07/03: factor型の扱い方を追加
    * 誤字・脱字は随時修正しております。
* 以下の内容は現在執筆中の内容の一部となります。
    * Song Jaehyun・矢内勇生『私たちのR: ぼくらがかんがえたさいきょうのRほん (仮)』(E-book)
        * 前半は「データハンドリング [基礎編: 抽出]」章、後半は「データハンドリング [基礎編: 拡張]」章に基づいており、factor型の扱いについては「データハンドリング [基礎編: factor型]」章を一部抜粋しております。
...
```{r pipe1, fig.width = 5, fig.cap = "パイプ演算子を使わない場合", echo = FALSE, fig.align = "center"}
...
```{r pipe2, out.width = "100%", fig.cap = "パイプ演算子を使う場合", echo = FALSE, fig.align = "center"}
...
話がずれますが、可視化における名目変数の扱いについて考えたいと思います。横軸、または縦軸が気温、成績、身長のような連続変数ではなく、都道府県や国、企業のような名目変数になる場合があります。たとえば、棒グラフの横軸は図\@ref(fig:factor1)のように、一般的に名目変数になる場合が多いです。

```{r factor1, fig.width = 6, fig.height = 3, message = FALSE, echo = FALSE, fig.cap = "横軸が名目変数の棒グラフ", fig.align = "center"}
df %>%
    group_by(Pref) %>%
...
```{r factor2, fig.width = 6, fig.height = 3, fig.cap = "Prefがcharacter型の場合 (1)", fig.align = "center"}
Score_df %>%
  ggplot() +
  geom_bar(aes(x = Pref, y = Score), stat = "identity") +
  labs(x = "都府県", y = "口コミ評価の平均値 (1~5)") +
...
```{r factor3, fig.width = 6, fig.height = 3, fig.cap = "Prefがcharacter型の場合 (2)", fig.align = "center"}
Score_df2 %>%
  ggplot() +
  geom_bar(aes(x = Pref, y = Score), stat = "identity") +
  labs(x = "都府県", y = "口コミ評価の平均値 (1~5)") +
...
```{r factor4, echo = FALSE, fig.width = 6, fig.height = 3, fig.cap = "Prefがfactor型の場合 (1)", fig.align = "center"}
Score_df_f1 %>%
  ggplot() +
  geom_bar(aes(x = Pref, y = Score), stat = "identity") +
  labs(x = "都府県", y = "口コミ評価の平均値 (1~5)") +
...
```{r factor5, echo = FALSE, fig.width = 3, fig.height = 3, fig.cap = "Kantoがnumeric型の場合", fig.align = "center"}
Score_df_f1 %>%
  ggplot() +
...
```{r factor6, echo = FALSE, fig.width = 3, fig.height = 3, fig.cap = "Kantoがfactor型の場合", fig.align = "center"}
Score_df_f1 %>%
  ggplot() +
  geom_bar(aes(x = Kanto, y = Score), stat = "identity") +
...
```{r merge-row, out.width = "80%", fig.cap = "行の結合", echo = FALSE, fig.align = "center"}
knitr::include_graphics("Figures/Handling1/Merge1.png")
```
...
```{r merge-col, out.width = "80%", fig.cap = "列の結合", echo = FALSE, fig.align = "center"}
knitr::include_graphics("Figures/Handling1/Merge2.png")
```
...
```{r inner-join, out.width = "80%", fig.cap = "`inner_join()`の仕組み", echo = FALSE, fig.align = "center"}
knitr::include_graphics("Figures/Handling1/Merge_Inner.png")
```
...
```{r full-join, out.width = "80%", fig.cap = "`full_join()`の仕組み", echo = FALSE, fig.align = "center"}
knitr::include_graphics("Figures/Handling1/Merge_Full.png")
```
...
```{r left-join, out.width = "80%", fig.cap = "`left_join()`の仕組み", echo = FALSE, fig.align = "center"}
knitr::include_graphics("Figures/Handling1/Merge_Left.png")
```
...
```{r right-join, out.width = "80%", fig.cap = "`right_join()`の仕組み", echo = FALSE, fig.align = "center"}
knitr::include_graphics("Figures/Handling1/Merge_Right.png")
```
